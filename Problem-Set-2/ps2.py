...
class RectangularRoom(object):
    ...
    def cleanTileAtPosition(self, pos):
        ...
        pos_x = int(pos.getX())
        pos_y = int(pos.getY())
        integer_position = (pos_x, pos_y)
        self.clean_tiles[integer_position] = 'clean'
    ...
    def getNumTiles(self):
        ...
        returns: an integer
        """
        total_tiles = int(self.width)*int(self.height)
        return total_tiles
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    for x in range(num_trials):
        ...
        for y in range(num_robots):
            sim_robots_list.append(robot_type(sim_room, speed))
            
        percent_cleaned = 0.0
        trial_steps_cleaned = []    
        
        while percent_cleaned < min_coverage:
            #anim.update(sim_room, sim_robots_list)    
            for robot in sim_robots_list:
                robot.updatePositionAndClean()
            percent_cleaned = (float(sim_room.getNumCleanedTiles()) / sim_room.getNumTiles())
            trial_steps_cleaned.append(percent_cleaned)
        #anim.done()
        
        trials_results.append(len(trial_steps_cleaned))
    
    average = float(sum(trials_results) / len(trials_results))
    return average            
