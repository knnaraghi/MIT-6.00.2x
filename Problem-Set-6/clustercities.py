...
class Cluster(object):
    ...
    def singleLinkageDist(self, other):
        ...
        dist = float(100000000)
        for x in self.points:
            for y in other.points:
                if x.distance(y) < dist:
                    dist = x.distance(y)               
        return dist
    ...
    def averageLinkageDist(self, other):
        ...
        if len(self.points) == 0 or len(other.points) == 0:
            ...
            return sum / (len(self.points)*len(other.points))
    def members(self):
...
def test():
    ...
    points = buildCityPoints('cityTemps.txt', True) #change to proper directory
...
#def test():
    #points = buildCityPoints('test.txt', False) #change to proper directory
